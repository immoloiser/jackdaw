...
fn finish_load_scene(world: &mut World, chosen: &std::path::Path) {
    ...
    if path.ends_with(".scene.json") {
        ...
        if !jsn.assets.material_definitions.is_empty() {
            world.resource_scope::<crate::material_definition::MaterialLibrary, ()>(
                |world, mut library| {
                    ...
                    for jsn_def in &jsn.assets.material_definitions {
                        ...
                        let Some(jsn_obj) = jsn_def.as_object() else {
                            continue;
                        };
                        let Some(serde_json::Value::String(name)) = jsn_obj.get("name") else {
                            continue;
                        };
                        if library.get_by_name(name).is_some() {
                            continue;
                        }

                        let Some(parent_path) = Path::new(&path).parent() else {
                            continue;
                        };
                        let mut deserializer_processor = JsnDeserializerProcessor {
                            asset_server,
                            parent_path,
                        };
                        let reflect_deserializer = ReflectDeserializer::with_processor(
                            &type_registry,
                            &mut deserializer_processor,
                        );
                        let Ok(standard_material) = reflect_deserializer.deserialize(jsn_def)
                        else {
                            continue;
                        };
                        let Some(standard_material) =
                            standard_material.try_downcast_ref::<StandardMaterial>()
                        else {
                            continue;
                        };

                        library.add(
                            crate::material_definition::MaterialDefinition::from_standard_material(
                                asset_server,
                                name.clone(),
                                standard_material,
                            ),
